...
export default (router) => {
    ...
    router.get('/users', isOwnerOrHasPermission('CAN_GET_USER', getUserById), (req, res) => {
        getUsers()
            .then(data => res.status(200).send(data))
            .catch(error => res.status(500).send({ stack: error.stack }));
    });

    router.get('/users/:id', isOwnerOrHasPermission('CAN_GET_USER', getUserById), (req, res) => {
    ...
    router.put('/users/:id', isOwnerOrHasPermission('CAN_EDIT_USER', getUserById), isValidForEditing, (req, res) => {
